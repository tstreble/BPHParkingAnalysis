...
int main(int argc, char** argv) {
  ...
  for (int iEntry = 0; iEntry < nentries ; iEntry++){
    ...
    if(_Muon_sel_index <0){ //Should implement something to avoid overlap with BToKmm final state (nMuon<2 won't work because only muons with pT>3 GeV are stored in default NanoAOD)
      tree_new->Fill();
      continue;
    }
